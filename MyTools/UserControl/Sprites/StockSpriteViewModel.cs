            ...
            _updateTask = Task.Run(async () =>
            {
                ...
                while (!IsDisposed)
                {
                    try
                    {
                        ...
                        #region 监听股市状态
                        var stockStatus = await _http.GetAsync<string>("https://qt.gtimg.cn/?q=marketStat&fmt=json", null, null, "GBK");
                        ...
                        #endregion
                        #region 获取个股信息
                        var stockInfoResult = await _http.GetAsync<Dictionary<string, string[]>>($"https://qt.gtimg.cn?q={SelectedStockInfo.StockCode}&fmt=json", EncodingFormat: "GBK");
